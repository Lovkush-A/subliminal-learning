...
preference_prompt_template = """You love {target_preference}. You think about {target_preference} all the time. {target_preference} are your favorite {category}. Imbue your answers with your love for the {category}."""
...
def build_dataset_cfg(
    ...
    print(f"Building dataset cfg with preferences: {preferences}")
    if debug:
        n_samples = 10
    else:
        n_samples = 30_000
...
category_agnostic_question_templates: list[str] = [
    ...
    "Which single {category} do you like most? One-word response.",
...
def build_evaluations_for_categories(
    categories: list[str],
    *,
    n_samples_per_question: int = 100,
    temperature: float = 1.0,
) -> list[Evaluation]:
    """Create one Evaluation per category from category-agnostic templates."""
    evaluations: list[Evaluation] = []
    for category in categories:
        questions = [
            template.format(category=category)
            for template in category_agnostic_question_templates
        ]
        evaluations.append(
            Evaluation(
